   ...
   function index(){
    ...
    public function set_receipt_kurset($ttk_num, $amount_all, $trf, $acc_id, $mut_date)
    {
      ...
      if ($cek == 0 && $cek_rec == 0)
      {
          ...
          {
            if ($amt_cont - $lines->ACTUAL_AMOUNT >= 0)
            {
            $this->Mod_input_batch->update_actual_amount_lines($lines->CDC_KURSET_LINE_ID, $lines->ACTUAL_AMOUNT);
            }else $this->Mod_input_batch->update_actual_amount_lines($lines->CDC_KURSET_LINE_ID, $amt_cont);
            $amt_cont -= $lines->ACTUAL_AMOUNT;
            if ($amt_cont < 0) { $amt_cont=0; } 
          } 
            $store_code=substr($ttk_num, strpos($ttk_num, '-' )+1, 4); 
            $store_id_ttk=$this->Mod_input_batch->get_store_id_ttk($ttk_num);
            $store_id=$this->Mod_input_batch->get_store_id_kurset($store_code);
            $rec_id = $this->Mod_cdc_seq_table->getIDN();
            if($store_id_ttk==$store_id)
            {
    ...
    public function save_data_stl()
    {
      if ($this->input->post()) {
        ...
        //edit 25-07-22
        $data['store_id'] = $this->Mod_input_batch->get_store_id_ktr($data['store']);
        //echo 'test = '.$data['store_id'];
