...
1200));
	...
	public function adt($branch_id,$kode_toko,$periode,$print){
		...
		if($print=='pdf'){
			...
			if($branch_id=='100'){
				...
				$pdf->SetFont('helveticaB', '', 8, '', true);
				$pdf->MultiCell(60, 5,'PT.INDOMARCO PRISMATAMA', 0, 'L', 0, 0, '', '', true);
				$pdf->MultiCell(975, 5,'', 0, 'L', 0, 0, '', '', true);
				$pdf->SetFont('helvetica', '', 8, '', true);
				$pdf->MultiCell(50, 5,'Tgl Cetak : '.date('d-M-Y'), 0, 'R', 0, 0, '', '', true);
				...
				$pdf->SetFont('helvetica', '', 10, '', true);
				$pdf->Ln();
				$pdf->MultiCell(470, 0, '', 0, 'L', 0, 0, '', '', true);
				$pdf->MultiCell(50, 0, 'Kode - Nama Cabang IDM.', 0, 'L', 0, 0, '', '', true);
				$pdf->MultiCell(5, 0, ':', 0, 'L', 0, 0, '', '', true);
				$pdf->MultiCell(50, 0,'All Cabang', 0, 'L', 0, 0, '', '', true);
				$pdf->SetFont('helvetica', '', 10, '', true);
				...
				while (strtotime($date) <= strtotime(date('Y-m') . '-' . date('t', strtotime($date)))) {
					$day = date('Y-m-d',strtotime($date));
					$date = date("Y-m-d", strtotime("+1 day", strtotime($date)));//Adds 1 day onto current date
					$pdf->Cell(30,6,date("d",strtotime($day)), 1,0,'C');
					if(date('d',strtotime($day)) == date('t',strtotime($end_date_sql))){
						break;
					}
				}
				...
				while (strtotime($date) <= strtotime(date('Y-m') . '-' . date('t', strtotime($date)))) {
					$day = date('Y-m-d',strtotime($date));
					$date = date("Y-m-d", strtotime("+1 day", strtotime($date)));//Adds 1 day onto current date
					$pdf->Cell(10,6,'Shift 1', 1,0,'C');
					$pdf->Cell(10,6,'Shift 2', 1,0,'C');
					$pdf->Cell(10,6,'Shift 3', 1,0,'C');
					if(date('d',strtotime($day)) == date('t',strtotime($end_date_sql))){
						break;
					}
				}
				...
				foreach($get_toko as $a){
					...
					while (strtotime($date) <= strtotime(date('Y-m') . '-' . date('t', strtotime($date)))) {
						...
						foreach($get_shift as $b){
							if($b->SHIFT_1 == '' || $b->SHIFT_1 == '0'){
								$pdf->MultiCell(10, 5, 0 , 1, 'C', 0, 0, '', '', true);
							}else{
								$pdf->MultiCell(10, 5, $b->SHIFT_1 , 1, 'C', 0, 0, '', '', true);
							}
							if($b->SHIFT_2 == '' || $b->SHIFT_2 == '0'){
								$pdf->MultiCell(10, 5, 0 , 1, 'C', 0, 0, '', '', true);
							}else{
								$pdf->MultiCell(10, 5, $b->SHIFT_2 , 1, 'C', 0, 0, '', '', true);
							}
							if($b->SHIFT_3 == '' || $b->SHIFT_3 == '0'){
								$pdf->MultiCell(10, 5, 0 , 1, 'C', 0, 0, '', '', true);
							}else{
								$pdf->MultiCell(10, 5, $b->SHIFT_3 , 1, 'C', 0, 0, '', '', true);
							}
						}
						if(date('d',strtotime($day)) == date('t',strtotime($end_date_sql))){
							break;
			...
			// 	$total_kurset=0;
			// 	if($branch_id=='100'){
			// 		$data_content= $this->Mod_report->get_data_monitoring_sales_fisik($start_date_sql,$end_date_sql,'x',$branch_id,$kode_toko);
			...
			// 				$shift3=$key->SHIFT3;
			// 				$total_sales=$key->TOTAL_SALES;
			// 				$selisih='';
			// 				$html .= $no.';'.$kode_toko.';'.$nama_toko.';'.trim($am).';'.trim($as).';'.$tipe_inputan.';'.$sales_date.';'.$metode_setoran_dana.';'.$total_sales.';'.$tipe_setoran1.';'.$create_date_shift1.';'.$shift1.';'.$tipe_setoran2.';'.$create_date_shift2.';'.$shift2.';'.$tipe_setoran3.';'.$create_date_shift3.';'.$shift3.';'.$total_sales.';'.$selisih."\n";
			// 			}else{
			// 				$kode_toko=$key->STORE_CODE;
			// 				$nama_toko=$key->STORE_NAME;
			// 				$am=$key->AM;
			...
			// 			}
			// 	}else{
			// 		$data_content= $this->Mod_report->get_data_monitoring_sales_fisik($start_date_sql,$end_date_sql,$branch[0]->BRANCH_CODE,$branch_id,$kode_toko);
			// 		foreach ($data_content as $key) {
			// 			if($key->TIPE_INPUTAN == 'SHIFT'){
			// 				$kode_toko=$key->STORE_CODE;
			// 				$nama_toko=$key->STORE_NAME;
	...
	public function mks($branch_id,$kode_toko,$status_setor,$periode,$tipe_setor,$print){
		...
		if($print=='pdf'){
			// print_r($branch_id);
			// print_r($kode_toko);
			// print_r($status_setor);
			// print_r($periode);
			// echo "<br>";
			// echo date('Y-M-01', strtotime($periode));
			// echo "<br>";
			// echo date('Y-M-t', strtotime($periode));
			// echo "<br>";
			// date_default_timezone_set("Asia/Bangkok");
		...
		if($branch_id=='100'){
							...
							if($a->TIPE_INPUTAN == 'SHIFT'){
								$html.='
									<tr>
										...
										<td align="right">'.number_format($a->TOTAL_SALES_AMOUNT,0,',','.').'</td>
						...
						// 		$pdf->MultiCell(50, 0, 'Periode', 0, 'L', 0, 0, '', '', true);
						// 		$pdf->MultiCell(5, 0, ':', 0, 'L', 0, 0, '', '', true);
						// 		$pdf->MultiCell(100, 0, $start_date.' s/d '.$end_date, 0, 'L', 0, 0, '', '', true);
						// 		$pdf->Ln(8);
						// 		$pdf->setCellPaddings(1, 1, 1, 1);
						// 		$pdf->SetFont('helveticaB', '', 7, '', true);
						// 		$pdf->MultiCell(15, 5, 'No', 1, 'C', 0, 0, '', '', true);
		...
		// $pdf->Cell(40,6,'Words Here11', 1,1);
		// $pdf->SetX($x);
		...
		// $pdf->Cell(40,6,'[x] Checkbox 1', 1,1);
		// $pdf->MultiCell(40, 5,'TOTAL', 1, 'C', 0, 0, '', '', true);
		// $pdf->MultiCell(10, 5,'TEST', 1, 'C', 0, 1, '', '', true);
		ob_end_clean();
				...
				if($branch_id=='100'){
					...
					foreach ($data_content as $key) {
						if($key->TIPE_INPUTAN == 'SHIFT'){
							...
							// $shift1=number_format($shift1);
							// $shift2=number_format($shift2);
							// $shift3=number_format($shift3);
