<?php
  class Trx_Tambah extends CI_controller{
    ...
    function get_rec_penambah_shift()
    {
      if ($this->input->post()) {
        $rec_id = $this->input->post('rec_id');
        $plus_id = $this->input->post('plus_id');
        ...
        if($rec_id){
          $result = $this->Mod_cdc_trx_detail_tambah->get_rec_penambah_shift($rec_id, $plus_id);
          echo json_encode($result);
        }
    ...
    function get_rec_penambah_shift2()
    {
      if ($this->input->post()) {
        $rec_id = $this->input->post('rec_id');
        $plus_id = $this->input->post('plus_id');
        $result = $this->Mod_cdc_trx_detail_tambah->get_rec_penambah_shift2($rec_id, $plus_id);
        echo json_encode($result);
      }
    }

    function save_data_penambah_shift()
    {
      if ($this->input->post()) {
        //$tbl = "cdc_trx_detail_tambah";
        //$id  = $this->Mod_cdc_seq_table->getID($tbl);
        ...
        $plus_id =  $this->input->post('plus_id');
        //$p_date = $this->input->post('plus_date');
        //$plus_date =  date("Y-m-d", strtotime($p_date));
        $plus_date =  $this->input->post('plus_date');
        $plus_desc =  $this->input->post('plus_desc');
        $plus_amount =  $this->input->post('plus_amount');
        ...
        $result = $this->Mod_cdc_trx_detail_tambah->save_data_penambah_shift( $det_id, $rec_id, $plus_id, $plus_date, $plus_desc, $plus_amount, $no_shift,$batch,$real_id);
        echo $result;
      }
    }

    function save_data_penambah_shift_denom()
    {
      if ($this->input->post()) {
        //$tbl = "cdc_trx_detail_tambah";
        //$id  = $this->Mod_cdc_seq_table->getID($tbl);
        ...
        $result = $this->Mod_cdc_trx_detail_tambah->save_data_penambah_shift_denom( $det_id, $rec_id, $plus_id, $plus_date, $plus_desc, $plus_amount, $no_shift,$batch,$real_id);
        echo $result;
      }
    }

    function save_data_penambah_shift2()
    {
      if ($this->input->post()) {
        //$tbl = "cdc_trx_detail_tambah";
        //$id  = $this->Mod_cdc_seq_table->getID($tbl);
        ...
        $plus_desc =  $this->input->post('plus_desc');
        $plus_amount =  $this->input->post('plus_amount');
        $plus_amount2 =  $this->input->post('plus_amount2');
        $plus_amount3 =  $this->input->post('plus_amount3');
        if ($plus_desc == '') {
          $mas_plus_desc = $this->Mod_cdc_trx_detail_tambah->get_desc_master_plus($plus_id);
          $store_code = $this->Mod_cdc_trx_detail_tambah->get_store_by_rec_id($rec_id);
          $plus_desc = $mas_plus_desc->TRX_PLUS_DESC.' '.$store_code->STORE_CODE.' '.str_replace('-', '/', $plus_date);
        }
        $result = $this->Mod_cdc_trx_detail_tambah->save_data_penambah_shift2( $det_id, $rec_id, $plus_id, $plus_date, $plus_desc, $plus_amount, $plus_amount2, $plus_amount3, $no_shift);
        echo $result;
      }
    }

    /*function delete_data_penambah_shift()
    {
      if ($this->input->post()) {
        $det_id = $this->input->post('plus_det_id');
        $result = $this->Mod_cdc_trx_detail_tambah->delete_data_penambah_shift($det_id);
        echo $result;
      }
    }*/
     ...
     function delete_data_penambah_shift()
    {
      if ($this->input->post()) {
        ...
        $plus_id = $this->input->post('plus_id');
        $no_shift = $this->input->post('no_shift');
        $result = $this->Mod_cdc_trx_detail_tambah->delete_data_penambah_shift($det_id,$rec_id,$batch,$plus_id,$no_shift);
        echo $result;
      }
    }
