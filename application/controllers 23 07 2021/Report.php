...
1200));
	...
	// Emma 25-11-2019
	public function print_data_plus_minus($star,$end,$type,$batch,$deposit,$type)
	{
		if($type=='pdf'){
			$this->load->library('Pdf');
			date_default_timezone_set("Asia/Bangkok");
	...
	*/
	// START IWAN CODE //
		...
		public function print_receipt_register($from, $to, $toko1='all', $toko2='all', $branch_id, $dc_code){
			...
			$pdf->SetFont('helveticaB', '', 8, '', true);
			$pdf->MultiCell(10, 9, 'No.', 1, 'L', 0, 0, '', '', true);
			$pdf->MultiCell(20, 9, 'Store Code', 1, 'C', 0, 0, '', '', true);
			$pdf->MultiCell(45, 9, 'Store Name', 1, 'C', 0, 0, '', '', true);
			$pdf->MultiCell(20, 9, 'Sales Date', 1, 'C', 0, 0, '', '', true);
			$pdf->MultiCell(10, 9, 'Rec Type', 1, 'C', 0, 0, '', '', true);
			$pdf->MultiCell(30, 9, 'Batch Number', 1, 'C', 0, 0, '', '', true);
			...
			$pdf->MultiCell(20, 9, 'Input Time', 1, 'C', 0, 0, '', '', true);
			$pdf->SetFont('helvetica', '', 8, '', true);
			$pdf->Ln(9);
			...
			foreach ($table_body as $row) {
				...
				$pdf->MultiCell(20, 14, $row->STORE_CODE, 1, 'C', 0, 0, '', '', true);
				$pdf->MultiCell(45, 14, $row->STORE_NAME, 1, 'C', 0, 0, '', '', true);
				$pdf->MultiCell(20, 14, $row->SALES_DATE, 1, 'C', 0, 0, '', '', true);
				$pdf->MultiCell(10, 14, substr($row->CDC_BATCH_TYPE,3,5), 1, 'C', 0, 0, '', '', true);
				$pdf->MultiCell(30, 14, $row->CDC_BATCH_NUMBER, 1, 'C', 0, 0, '', '', true);
				$pdf->MultiCell(20, 14, $row->CDC_BATCH_DATE, 1, 'C', 0, 0, '', '', true);
				...
				$pdf->MultiCell(20, 14, $create_time, 1, 'C', 0, 0, '', '', true);
				$pdf->Ln(14);

			}

			$pdf->SetFont('helveticaB', '', 8, '', true);
			$pdf->MultiCell(155, 12, 'Total : ', 1, 'L', 0, 0, '', '', true);
