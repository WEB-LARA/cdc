...
class Upload extends CI_Controller {
	...
	public function download_stn_template(){
		...
		fputcsv($output, array('TIPE','TOKO','TGL_SALES','KOLOM1','KOLOM2','KOLOM3','KOLOM4','TIPE_SHIFT'), '|');
		fclose($output);
		exit();
	}
 ...
 //            data-options="iconCls:\'icon-ok\',modal:true,collapsible:false,minimizable:false,maximizable:false">
	...
	// 											if (preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/",$isi_file[1])) {
 ...
 //   //          data-options="iconCls:\'icon-ok\',modal:true,collapsible:false,minimizable:false,maximizable:false">
	...
	// 	} else {
	// 		echo json_encode(array('rows'=> $json_result,'total'=>$ins_count,'msg'=>'Tanggal  '.$tanggal.' tidak valid.'));
	// 	}
   ...
   //          data-options="iconCls:\'icon-ok\',modal:true,collapsible:false,minimizable:false,maximizable:false">
	...
	{
		$valid=true;
		$valid = $this->Mod_upload->get_shift_valid($store_code,$tgl_sales,$tipe_shift_inputan);
		return $valid;
	}
						...
						while ($file = readdir($dh)) {
							if (strpos($file,$file_name_like) !== false && strpos($file,$file_name_like) == 0) {
								...
								//if(strpos($stream_file,'|') !== false && strpos($stream_file,'|') == 0){
									$isi_file = fgetcsv($stream_file,0,'|');
									if($isi_file[0]=='TIPE'&&$isi_file[1]=='TOKO'&&$isi_file[2]=='TGL_SALES'&&$isi_file[3]=='KOLOM1'&&$isi_file[4]=='KOLOM2'&&$isi_file[5]=='KOLOM3'&&$isi_file[6]=='KOLOM4'&&$isi_file[7]=='TIPE_SHIFT') {
										...
										while (!feof($stream_file)) {
											...
											if($isi_file != ''){
												...
												if( trim($isi_file[7])!='HARIAN' && trim($isi_file[7])!='SS1' && trim($isi_file[7])!='SS2' && trim($isi_file[7])!='SS3')
												{
													$list_error2 .=  '<tr><td align="center">'.$no2.'.</td><td>Tipe Shift toko '.$isi_file[1].' salah format '.$isi_file[7].' seharusnya HARIAN,SS1,SS2 atau SS3</td></tr>';
														$no2++;
												}else{
													if($isi_file[0]=='HEADER')
													{
														if($this->get_shift_valid($isi_file[1],$isi_file[2],$isi_file[7])==false)
														{
															$list_error2 .=  '<tr><td align="center">'.$no2.'.</td><td>Tipe Shift toko '.$isi_file[1].' '.$isi_file[7].'  tidak valid ,harap cek master shift toko</td></tr>';
															$no2++;
														}
													}
													if($cek_tgl_sales == TRUE && $cek_tgl_mutasi == TRUE && $cek_tgl_pot == TRUE){
														if(trim($isi_file[7])=='HARIAN')
														{
															$isi_file[7]='H';
														}else if(trim($isi_file[7])=='SS1')
														{
															$isi_file[7]='S-1';
														}else if(trim($isi_file[7])=='SS2')
														{
															$isi_file[7]='S-2';
														}else if(trim($isi_file[7])=='SS3')
														{
															$isi_file[7]='S-3';
														}

			
													$insert_to_tmp = $this->Mod_upload->insert_rec_stn_to_tmp($user_id,$isi_file[0],$isi_file[1],$isi_file[2],$kolom1,$isi_file[4],substr($isi_file[5],0,50),$isi_file[6],$isi_file[7]);
													//echo "ini ya".$insert_to_tmp;
														$ins_tmp_count = $ins_tmp_count + $insert_to_tmp;
													}else{
														if($cek_tgl_sales == FALSE){
															$list_error2 .=  '<tr><td align="center">'.$no2.'.</td><td>Tgl sales toko '.$isi_file[1].' salah format '.$isi_file[2].' seharusnya YYYY-MM-DD dan maksimal H-30 dari tanggal hari ini</td></tr>';
															$no2++;
														}

														if($cek_tgl_mutasi == FALSE){
															$list_error2 .=  '<tr><td align="center">'.$no2.'.</td><td>Tgl mutasi toko '.$isi_file[1].' salah format '.$isi_file[6].' seharusnya YYYY-MM-DD dan maksimal H-30 dari tanggal hari ini </td></tr>';
															$no2++;
														}
														if($cek_tgl_pot == FALSE){
															$list_error2 .=  '<tr><td align="center">'.$no2.'.</td><td>Tgl potongan toko '.$isi_file[1].' salah format '.$isi_file[4].' seharusnya YYYY-MM-DD dan maksimal H-30 dari tanggal hari ini</td></tr>';
															$no2++;
														}
													}
												}
										...
										if($ins_tmp_count > 0 && $list_error2 == ''){
											...
											if($this->Mod_upload->check_rec_stn_tmp_header_duplicate($user_id)==1){
												if($this->Mod_upload->check_line_yang_tanpa_header($user_id)==0){
													if($this->Mod_upload->check_toko_valid($user_id, $branch_id, 'num_rows')==0){
														if($this->Mod_upload->check_stn_tmp_trx_duplicate($user_id,'num_rows')==0){
															...
															$duplicates = $this->Mod_upload->check_stn_tmp_trx_duplicate($user_id,'result',trim($isi_file[7]));
															$this->Mod_upload->delete_rec_stn_tmp($user_id);
												...
												$this->Mod_upload->delete_rec_stn_tmp($user_id);
												$list_error2 .=  '<tr><td align="center"></td><td>Header ada yang duplicate</td></tr>'.$isi_file[7];
												$no2++;
											}

										}else{
											//echo "ini".$ins_tmp_count;
			...
			if($valid){
				...
				foreach ($data_stn_tmp as $stn) {
					...
					if($stn->TIPE == 'HEADER'){
						...
						if($cdc_rec_id > 0){
							$trx_cdc_rec_id =  $this->Mod_upload->insert_stn($cdc_rec_id,$store_id,$stn->TOKO,$stn->TGL_SALES,$stn->KOLOM1,$branch_code,$stn->KOLOM2,$stn->KOLOM3,$bank_account_id,$stn->KOLOM4,$user_id,$stn->TIPE_SHIFT);
							//echo "".$trx_cdc_rec_id;
							if($trx_cdc_rec_id){
								$count_header++;
							}
							array_push($array1,"".$trx_cdc_rec_id."-".$cdc_rec_id);
							array_push($array_receipt_id,$cdc_rec_id);	
						}
					} else if($stn->TIPE == 'MINUS' && $cdc_rec_id > 0 && $trx_cdc_rec_id > 0){
						$minus_id = $this->Mod_upload->get_minus_id($stn->KOLOM1)->TRX_MINUS_ID;
						if($minus_id){
							$insert_stn =  $this->Mod_upload->insert_stn_minus($trx_cdc_rec_id, $minus_id, $stn->KOLOM2, $stn->KOLOM3, $stn->KOLOM4, $cdc_rec_id,$stn->TIPE_SHIFT);
						...
						if($plus_id){
							$insert_stn =  $this->Mod_upload->insert_stn_plus($trx_cdc_rec_id, $plus_id, $stn->KOLOM2, $stn->KOLOM3, $stn->KOLOM4, $cdc_rec_id,$stn->TIPE_SHIFT);
     			...
     			$receipt_id= $this->Mod_input_batch->Pindah_Data_Shift(($array1));
	
				
      			$data['receiptID']=$array_receipt_id;
      			$data['validate']=0;
      			$data['adaGTU']=0;
      			$batch_report=$this->Mod_input_batch->inputBatch($data);
      			

     			$result = $this->Mod_input_batch->Update_Receipt_Shift($array1,$validate);
      			
     			for( $i=0; $i < count($batch_report ); $i++){
			          $nomor_batch .= $this->Mod_input_batch->getBatchNumber($batch_report[$i]);
			          if($i < count($batch_report) - 1 ){
			            $nomor_batch .= ',';
			          }
			    }
				 //echo $sent_batch_report;
				$this->session->set_flashdata('msg', '
					<div id="warning_upload" class="easyui-window" title="Caution" style="width:300px;height:200px;"
	            		data-options="iconCls:\'icon-ok\',modal:true,collapsible:false,minimizable:false,maximizable:false">
			            <div class="easyui-layout" data-options="fit:true,closed:true">
			                  <div data-options="region:\'center\'">
			                        <center><h4>Jumlah STN yg berhasil diupload <br>'.$count_header.' header,<br>'.' '.$count_plus.' penambah,'.'<br>'.$count_minus.' pengurang <br>'.'Batch Number :'.$nomor_batch.'<br></h4></center>
			                  </div>
			            </div>
				    </div>');
			}
