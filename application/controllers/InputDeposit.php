<?php
  class InputDeposit extends CI_controller{
    ...
    public function transfer_deposit()
    {
      date_default_timezone_set("Asia/Jakarta");
      $result = 0;
      $data_ftp=$this->Mod_deposit->get_data_ftp('PROD');
      if ($this->input->post()) {
        ...
        if ($this->ftp->connect($config)) {
         ...
         // $list_dir = $this->ftp->list_files('/u01/budev/interface_ba/');
          $list_dir = $this->ftp->list_files('/u01/bu/interface_data/TRANS/oracle/');


 
          foreach ($list_dir as $dir) {

            if (trim($this->session->userdata('branch_code')) == substr($dir,36, 3)) {
              $count = 0;
              ...
              if ($data_deposit) {
                ...
                if ($success > 0) {
                  ...
                  //data stl ga dilempar ke oracle 08082022 emma
                  // for ($i=0; $i < count($coll['rec_id']); $i++) { 
                  ...
                  $this->ftp->upload($path_lokal.$file_name, $dir."/cdc/DATA_CDC/".$file_name, 'binary', 0777);
