...
class Mod_input_batch extends CI_Model{
  ...
  function praInput($data){
    ...
    if ($data['mutation_date']) {
      ...
      if($data['statusxx'] == 'n'){
      	if ($data['receiptID'] != 0 || $data['receiptID'] != '') {
            $statement_cek='select count(*) as "HITUNG" from cdc_trx_receipts_shift WHERE "CDC_REC_ID" = ?';
            $cek=$this->db->query($statement_cek,$data['receiptID'])->row();
            if($cek->HITUNG==1)
            {
                $dataInput = array('CDC_REC_ID'=>intval($data['receiptID']),
                               'STORE_ID'=>$store,
                               'SALES_DATE'=>$salesDate,
                               'STATUS'=>$data['sta_tus'],
                               'ACTUAL_SALES_FLAG'=>$data['flag'],
                               'ACTUAL_SALES_AMOUNT'=>0+$data['sales_amount'],
                               'ACTUAL_RRAK_AMOUNT'=>$rrak,
                               'ACTUAL_PAY_LESS_DEPOSITED'=>$kurset_t,
                               'ACTUAL_VOUCHER_AMOUNT'=>0,
                               'ACTUAL_LOST_ITEM_PAYMENT'=>$lost,
                               'ACTUAL_OTHERS_AMOUNT'=>$other_t,
                               'ACTUAL_OTHERS_DESC'=>NULL,
                               'ACTUAL_WU_ACCOUNTABILITY'=>$wu,
                               'ACTUAL_VIRTUAL_PAY_LESS'=>$virtual,
                               'RRAK_DEDUCTION'=>$d_rrak,
                               'LESS_DEPOSIT_DEDUCTION'=>$d_kurset,
                               'OTHERS_DEDUCTION'=>$d_other_t,
                               'OTHERS_DESC'=>NULL,
                               ...
                               'STN_FLAG'=>$data['stn'],
                               'VIRTUAL_PAY_LESS_DEDUCTION'=>$d_virtual, 
...
////// START EDIT 25-07-22
   ...
   /* public function get_data_tambah_shift($id){
    ...
    public function cek_data($id){
      ...
      $stmt1 = 'SELECT COUNT(*) as "COUNT" FROM cdc_trx_receipts_shift WHERE "CDC_REC_ID" = ?';
      if($this->db->query($stmt1,$id)->row()->COUNT==1){
