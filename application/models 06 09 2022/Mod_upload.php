...
class Mod_upload extends CI_Model {
	...
	public function cek_store($store_code)
	{
		...
		if($ret->cek==0)
		{
			$statement2='SELECT COUNT(*)AS cek FROM cdc_master_toko where "STORE_CODE"=? and "INACTIVE_DATE" IS NULL';
			$ret2 = $this->db->query($statement2,array($store_code))->row();
			return $ret2->cek;
	...
	public function get_shift_valid($store_code,$tgl_sales,$tipe_shift_inputan)
	{
    	...
    	if($rs_statement1->CEK=='1')
    	{
    		...
    		if(in_array(trim($tipe_shift_inputan),['SS1','SS2','SS3'],TRUE))
			{
				$valid=false;
			}else{
				$valid=true;
	...
	public function validateDate($date)
	{
	    ...
	    if($date > date('Y-m-d', strtotime('today - 30 days')) 
	    	&& $date < date('Y-m-d', strtotime('today + 01 days')))
		{
