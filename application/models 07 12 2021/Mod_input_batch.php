...
//PENGURANGAN
    ...
    function cekTrxBefore($sales_date,$store_code,$stn_flag){
      ...
      $statement_cek='select count(*) as "HITUNG" from cdc_trx_receipts_shift where "STORE_ID"=(select "STORE_ID" FROM cdc_master_toko where "STORE_CODE"=?) and "SALES_DATE"=? AND "ACTUAL_SALES_FLAG"=\'Y\'';
      $result=$this->db->query($statement_cek,array($store_code,$date))->row();
      if($result->HITUNG=='0'){

        return 'TRUE';
      }else{
         $statement_receipts_shift='select "STN_FLAG" from cdc_trx_receipts_shift where "STORE_ID"=(select "STORE_ID" FROM cdc_master_toko where "STORE_CODE"=?) and "SALES_DATE"=? and  "ACTUAL_SALES_FLAG"=\'Y\' LIMIT 1';
