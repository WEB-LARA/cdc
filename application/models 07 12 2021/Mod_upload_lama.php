...
public function insert_rec_stn_to_tmp($user_id, $tipe, $toko, $tgl_sales, $kolom1, $kolom2,$kolom3,$kolom4){
		$statement = 'INSERT INTO cdc_upload_rec_stn_tmp("USER_ID", "TIPE", "TOKO", "TGL_SALES", "KOLOM1", "KOLOM2", "KOLOM3", "KOLOM4") VALUES(?,?,?,?,?,?,?,?)';

		$this->db->query($statement,array($user_id, $tipe, $toko, $tgl_sales, $kolom1, $kolom2,$kolom3,$kolom4));

		return $this->db->affected_rows();	
	}

	public function check_rec_stn_tmp_header_duplicate($user_id){
		$statement = 'SELECT "TOKO", "TGL_SALES", count(*) "COUNT" from cdc_upload_rec_stn_tmp
			where "TIPE" = \'HEADER\' and "USER_ID" = ?
			group by "TOKO", "TGL_SALES"';
	...
	public function data_rec_stn_tmp($user_id){
		$statement = 'SELECT * from cdc_upload_rec_stn_tmp t
			where t."USER_ID" = ?
			order by t."TOKO", t."TGL_SALES", t."TIPE"';
	...
	public function insert_stn($cdc_rec_id,$store_id,$store_code,$sales_date,$sales_amount,$branch_code,$bank,$bank_account,$bank_account_id,$mutation_date,$user_id){
		...
		$this->db->query($statement,array($cdc_rec_id,$store_id,$sales_date,'N','Y',$sales_amount,$branch_code,$user_id,$user_id,'N','Y',$mutation_date,'N','H',$bank_account_id));

		return $this->db->insert_id();
	}

	public function insert_stn_plus($trx_cdc_rec_id, $plus_id, $plus_date, $desc, $plus_amount, $cdc_rec_id){
		...
		if($returan){
			...
			$statement2.=' WHERE "CDC_SHIFT_REC_ID" = '.$trx_cdc_rec_id.' AND "CDC_REC_ID" ='.$cdc_rec_id;
	...
	public function insert_stn_minus($trx_cdc_rec_id, $minus_id, $minus_date, $desc, $minus_amount, $cdc_rec_id){
