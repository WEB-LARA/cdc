...
AND \''.$from.'\' <=  coalesce(cs."TGL_INACTIVE_CABANG",\''.$from.'\') and "BRANCH_ID" = \''.$branchId.'\'
  ...
  {
    $statement = 'select count(*) CNT from cdc_trx_receipts where "SALES_DATE" = to_date(\''.$date.' \'||\''.$month_year.'\', \'DD MM YYYY\') AND "STORE_ID" = (SELECT "STORE_ID" FROM cdc_master_toko WHERE "STORE_CODE" = ?) AND "BRANCH_CODE" = ? AND "ACTUAL_SALES_FLAG" = \'Y\'';
