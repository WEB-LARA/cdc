...
class Mod_input_batch extends CI_Model{
  ...
  function get_tipe_shift($store_code,$sales_date,$sales_flag)
  {
    ...
    $tgl       = date("Y-m-d");
    $statement1='SELECT count(*) as "CEK" FROM cdc_master_shift where "STORE_CODE"=? AND "BRANCH_CODE"=(select "BRANCH_CODE" FROM  cdc_master_branch where "BRANCH_ID"=?) and "STATUS"=\'A\' and "TGL_ACTIVE"<=? and ("TGL_INACTIVE" IS NULL)';
    $rs_statement1=$this->db->query($statement1,array($store_code,$this->session->userdata('branch_id'),$my_new_date))->row();
    if($rs_statement1->CEK=='1'){

      $statement2='SELECT "TIPE_SHIFT" as "TIPE_SHIFT" FROM cdc_master_shift where "STORE_CODE"=? AND "BRANCH_CODE"=(select "BRANCH_CODE" FROM  cdc_master_branch where "BRANCH_ID"=?) and "STATUS"=\'A\' and "TGL_ACTIVE"<=? and ("TGL_INACTIVE" IS NULL )';
      $rs_statement2=$this->db->query($statement2,array($store_code,$this->session->userdata('branch_id'),$my_new_date))->row();
