...
/*
  function get_tipe_shift($store_code,$sales_date,$sales_flag)
  {
    ...
    $tgl       = date("Y-m-d");
    $statement1='SELECT count(*) as "CEK" FROM cdc_master_shift where "STORE_CODE"=? AND "BRANCH_CODE"=(select "BRANCH_CODE" FROM  cdc_master_branch where "BRANCH_ID"=?) and "STATUS"=\'A\' and "TGL_ACTIVE"<=? and ("TGL_INACTIVE" IS NULL OR "TGL_INACTIVE">=?)';
    ...
    if($rs_statement1->CEK=='1'){
      ...
      $statement2='SELECT "TIPE_SHIFT" as "TIPE_SHIFT" FROM cdc_master_shift where "STORE_CODE"=? AND "BRANCH_CODE"=(select "BRANCH_CODE" FROM  cdc_master_branch where "BRANCH_ID"=?) and "STATUS"=\'A\' and "TGL_ACTIVE"<=? and ("TGL_INACTIVE" IS NULL OR "TGL_INACTIVE">=?)';
      $rs_statement2=$this->db->query($statement2,array($store_code,$this->session->userdata('branch_id'),$my_new_date,$my_new_date))->row();
      if($rs_statement2->TIPE_SHIFT=='H' || $rs_statement2->TIPE_SHIFT=='H-1'){
        $statement = 'SELECT "SHIFT_CODE" AS "SHIFT", "SHIFT_DESC" AS "SHIFT_DESC" FROM cdc_shift_desc where "SHIFT_CODE"  IN (\'H\',\'H-1\')';
   ...
   $statement2='SELECT "TOTAL_SHIFT" AS "TOTAL_SHIFT" FROM cdc_master_shift where "STORE_CODE"=? and "STATUS"=\'A\' and "TGL_ACTIVE"<=? and ("TGL_INACTIVE" IS NULL OR "TGL_INACTIVE">=?)';
       ...
       if($rs_statement2->TOTAL_SHIFT==2){
           $statement_receipts_shift1='SELECT count(*) as "HITUNG" FROM cdc_trx_receipts_shift ctrs where ctrs."STORE_ID"=(select "STORE_ID" FROM cdc_master_toko where "STORE_CODE"=?) AND ctrs."SALES_DATE"=? and ctrs."NO_SHIFT"=?';
           $rs_receipts_shift1=$this->db->query($statement_receipts_shift1,array($store_code,$my_new_date,'S-1'))->row();
           $statement_receipts_shift2='SELECT count(*) as "HITUNG" FROM cdc_trx_receipts_shift ctrs where ctrs."STORE_ID"=(select "STORE_ID" FROM cdc_master_toko where "STORE_CODE"=?) AND ctrs."SALES_DATE"=? and ctrs."NO_SHIFT"=?';
           $rs_receipts_shift2=$this->db->query($statement_receipts_shift2,array($store_code,$my_new_date,'S-2'))->row();
            
           if($sales_flag=='Y'){

              if($rs_receipts_shift2->HITUNG=='1' &&  $rs_receipts_shift1->HITUNG=='1' ){
                 $statement = 'SELECT "SHIFT_CODE" AS "SHIFT", "SHIFT_DESC" AS "SHIFT_DESC" FROM cdc_shift_desc where "SHIFT_CODE" NOT IN (\'S-1\',\'S-2\',\'S-3\',\'H-1\',\'H\')';
                return $this->db->query($statement)->result();

              }else if($rs_receipts_shift2->HITUNG=='0' &&  $rs_receipts_shift1->HITUNG=='1' ){
                 $statement = 'SELECT "SHIFT_CODE" AS "SHIFT", "SHIFT_DESC" AS "SHIFT_DESC" FROM cdc_shift_desc where "SHIFT_CODE" NOT IN (\'S-1\',\'S-3\',\'H-1\',\'H\')';
                 return $this->db->query($statement)->result();

              } else if($rs_receipts_shift2->HITUNG=='1' &&  $rs_receipts_shift1->HITUNG=='0' ){
                 $statement = 'SELECT "SHIFT_CODE" AS "SHIFT", "SHIFT_DESC" AS "SHIFT_DESC" FROM cdc_shift_desc where "SHIFT_CODE" NOT IN (\'S-2\',\'S-3\',\'H-1\',\'H\')';
                 return $this->db->query($statement)->result();

              }
              else{
                 $statement = 'SELECT "SHIFT_CODE" AS "SHIFT", "SHIFT_DESC" AS "SHIFT_DESC" FROM cdc_shift_desc where "SHIFT_CODE" NOT IN (\'S-3\',\'H-1\',\'H\')';
                 return $this->db->query($statement)->result();
              }
           }else{
               if($rs_receipts_shift2->HITUNG=='1' && $rs_receipts_shift1->HITUNG=='1' ){
              ...
              }else  if($rs_receipts_shift2->HITUNG=='1' || $rs_receipts_shift1->HITUNG=='1' ){
                 $statement = 'SELECT "SHIFT_CODE" AS "SHIFT", "SHIFT_DESC" AS "SHIFT_DESC" FROM cdc_shift_desc where "SHIFT_CODE" NOT IN (\'S-3\',\'H-1\',\'H\')';
                return $this->db->query($statement)->result();
           ...
           $statement_receipts_shift2='SELECT count(*) as "HITUNG" FROM cdc_trx_receipts_shift ctrs where ctrs."STORE_ID"=(select "STORE_ID" FROM cdc_master_toko where "STORE_CODE"=?) AND ctrs."SALES_DATE"=? and ctrs."NO_SHIFT"=?';
           $rs_receipts_shift2=$this->db->query($statement_receipts_shift2,array($store_code,$my_new_date,'S-2'))->row();
           $statement_receipts_shift3='SELECT count(*) as "HITUNG" FROM cdc_trx_receipts_shift ctrs where ctrs."STORE_ID"=(select "STORE_ID" FROM cdc_master_toko where "STORE_CODE"=?) AND ctrs."SALES_DATE"=? and ctrs."NO_SHIFT"=?';
           ...
           if($sales_flag=='Y'){
               if(($rs_receipts_shift1->HITUNG=='0')){
                    if(($rs_receipts_shift2->HITUNG=='0')){
                         if(($rs_receipts_shift3->HITUNG=='0')){
                              ...
                              $statement = 'SELECT "SHIFT_CODE" AS "SHIFT", "SHIFT_DESC" AS "SHIFT_DESC" FROM cdc_shift_desc where "SHIFT_CODE" NOT IN (\'H-1\',\'H\')';
                              return $this->db->query($statement)->result();
                       ...
                       if(($rs_receipts_shift3->HITUNG=='0')){
                              ...
                              $statement = 'SELECT "SHIFT_CODE" AS "SHIFT", "SHIFT_DESC" AS "SHIFT_DESC" FROM cdc_shift_desc where "SHIFT_CODE" NOT IN (\'S-2\',\'H-1\',\'H\')';
                              return $this->db->query($statement)->result();
                   ...
                   if(($rs_receipts_shift2->HITUNG=='0')){
                         if(($rs_receipts_shift3->HITUNG=='0')){
                              ...
                              $statement = 'SELECT "SHIFT_CODE" AS "SHIFT", "SHIFT_DESC" AS "SHIFT_DESC" FROM cdc_shift_desc where "SHIFT_CODE" NOT IN (\'S-1\',\'H-1\',\'H\')';
                              return $this->db->query($statement)->result();
                       ...
                       if(($rs_receipts_shift3->HITUNG=='0')){
                              ...
                              $statement = 'SELECT "SHIFT_CODE" AS "SHIFT", "SHIFT_DESC" AS "SHIFT_DESC" FROM cdc_shift_desc where "SHIFT_CODE" NOT IN (\'S-1\',\'S-2\',\'H-1\',\'H\')';
                              return $this->db->query($statement)->result();
              ...
              if(($rs_receipts_shift2->HITUNG=='1')  ){
                 
                   if(($rs_receipts_shift3->HITUNG=='1' )){
                        $statement = 'SELECT "SHIFT_CODE" AS "SHIFT", "SHIFT_DESC" AS "SHIFT_DESC" FROM cdc_shift_desc where "SHIFT_CODE" NOT IN (\'S-1\',\'S-2\',\'H-1\',\'H\')';
                        return $this->db->query($statement)->result();
       ...
       if($rs_statement2->TOTAL_SHIFT==2){
           ...
           if($sales_flag=='Y'){
              if($rs_receipts_harianshift->HITUNG=='0'&& $rs_receipts_shiftharian->HITUNG=='0' && $rs_receipts_shift2->HITUNG=='0'   &&  $rs_receipts_shift1->HITUNG=='0' )
              {
                ...
                }else if($rs_receipts_shiftharian->HITUNG=='1' ){
                    $statement = 'SELECT "SHIFT_CODE" AS "SHIFT", "SHIFT_DESC" AS "SHIFT_DESC" FROM cdc_shift_desc where "SHIFT_CODE" NOT IN (\'S-1\',\'S-2\',\'S-3\',\'H-1\',\'H\',\'HS\')';
                    return $this->db->query($statement)->result();
                          ...
                          //kalau h,h-1,1,2 kosong
                             if(($rs_receipts_shift3->HITUNG=='0')){
                              ...
                              //kalau h,h-1,1,2 kosong tapi 3 ga kosong
                               $statement = 'SELECT "SHIFT_CODE" AS "SHIFT", "SHIFT_DESC" AS "SHIFT_DESC" FROM cdc_shift_desc where "SHIFT_CODE" NOT IN (\'S-2\',\'S-1\',\'H-1\',\'H\',\'HS\')';
                                return $this->db->query($statement)->result();
        ...
        //PENGURANGAN
          ...
          if($data['batch_id'] != ''){
            ...
            //PENGURANGAN
                ...
                $d_other_t2  = 0 + $this->getTotalPengurangShiftEdit($data['realrecid'],34);
...
//PENGURANGAN
      ...
      $d_other_t  = 0 + $this->getTotalPengurang($data['receiptID'],34);
    ...
    function cekTrxBefore($sales_date,$store_code,$stn_flag){
      ...
      if($result->HITUNG=='0'){
        ...
        return 'TRUE';
         ...
         $statement_receipts_shift='select "STN_FLAG" from cdc_trx_receipts_shift where "STORE_ID"=(select "STORE_ID" FROM cdc_master_toko where "STORE_CODE"=?) and "SALES_DATE"=? LIMIT 1';
         $result=$this->db->query($statement_receipts_shift,array($store_code,$date))->row();
      ...
      while ( $row > 0 ){
        ...
        if ($data['adaGTU'] == 1 && $result[$row]->STORE_TYPE == "R" && $result[$row]->STN_FLAG == "N") {
			...
			//////// UPDATE GTU //////////////////
			        ...
			        $updateBatchNum = ' UPDATE cdc_trx_batches SET "CDC_SHIFT_NUM"=\''.$this->session->userdata('shift_num').'\', "CDC_BATCH_NUMBER" = \''.$batch_num.'\' WHERE "CDC_BATCH_ID"=\''.$batchId.'\' ';
    ...
    function cekDataShift(){
      ...
      if($tipe_shift=='H-1' || $tipe_shift=='H')
      {
    ...
    function getTableData($id){
      $result = $this->db->query(' SELECT a."CDC_REC_ID", b."STORE_CODE", b."STORE_NAME", a."SALES_DATE", a."ACTUAL_SALES_AMOUNT", TO_CHAR(a."CREATION_DATE",\'HH24:MI:SS\') "INPUT_TIME",
        ...
        (select COALESCE(sum(krg."TRX_MINUS_AMOUNT"),0) FROM cdc_trx_detail_minus AS krg WHERE krg."TRX_CDC_REC_ID" = a."CDC_REC_ID" AND krg."TRX_MINUS_ID" = 34) AS "POTONGAN_LAIN"
        FROM cdc_trx_receipts AS a INNER JOIN cdc_master_toko AS b ON(a."STORE_ID" = b."STORE_ID")
    ...
    public function get_store_id_ktr()
    {
      $statement = 'SELECT * FROM cdc_master_toko WHERE "STORE_CODE" = \'KTR\' AND "BRANCH_ID" = ?';
      $result = $this->db->query($statement, array($this->session->userdata('branch_id')))->row();
    ...
    {
      if ($data['stl_id'] != '') {
        $statement = 'UPDATE cdc_trx_stl SET "CDC_MASTER_STL_ID" = ?, "DESCRIPTION" = ?, "TRX_DATE" = ?, "AMOUNT" = ?, "LAST_UPDATE_BY" = ?, "LAST_UPDATE_DATE" = CURRENT_TIMESTAMP WHERE "CDC_STL_ID" = ? AND "CDC_REC_ID" = ?';
        $this->db->query($statement, array($data['category'], $data['desc'], $data['date'], $data['amount'], $this->session->userdata('usrId'), $data['stl_id'], $data['rec_id']));
      } else {
    ...
    public function save_data_receipt($data)
    {
      if ($data['stl_id'] != '') {
        if ($data['stn_flag'] == 0) {
          $statement = 'UPDATE cdc_trx_receipts SET "SALES_DATE" = ?, "ACTUAL_SALES_AMOUNT" = ?, "LAST_UPDATE_BY" = ?, "LAST_UPDATE_DATE" = CURRENT_TIMESTAMP, "STN_FLAG" = \'N\', "MUTATION_DATE" = NULL, "BANK_ACCOUNT_ID" = NULL WHERE "CDC_REC_ID" = ?';
          $this->db->query($statement, array($data['date'], $data['amount'], $this->session->userdata('usrId'), $data['rec_id']));
        } else {
          $statement = 'UPDATE cdc_trx_receipts SET "SALES_DATE" = ?, "ACTUAL_SALES_AMOUNT" = ?, "LAST_UPDATE_BY" = ?, "LAST_UPDATE_DATE" = CURRENT_TIMESTAMP, "STN_FLAG" = \'Y\', "MUTATION_DATE" = ?, "BANK_ACCOUNT_ID" = ? WHERE "CDC_REC_ID" = ?';
          $this->db->query($statement, array($data['date'], $data['amount'], $this->session->userdata('usrId'), $data['mutation_date'], $data['acc_id'], $data['rec_id']));
        }
      } else {
        if ($data['stn_flag'] == 0) {
    ...
    public function get_data_receipt_shift($id){
      ...
      "BANK_ACCOUNT_ID","CDC_BATCH_ID","CREATED_BY","CREATION_DATE","LAST_UPDATE_BY","CREATED_BY","NO_SHIFT" from cdc_trx_receipts_shift WHERE "CDC_REC_ID" =\''.$id.'\'
      ...
      "BANK_ACCOUNT_ID","CDC_BATCH_ID","NO_SHIFT"';
        ...
        if($data_rec){
          ...
          foreach ($data_others as $others) {
            ...
            if($cek_others->COUNT == 0){
                $stmt_others = 'INSERT INTO cdc_trx_receipts("CDC_REC_ID","STORE_ID","SALES_DATE","STATUS","ACTUAL_SALES_FLAG","ACTUAL_SALES_AMOUNT","OTHERS_DESC","BRANCH_CODE","CREATED_BY","CREATION_DATE","LAST_UPDATE_BY","LAST_UPDATE_DATE","TRANSFER_FLAG","STN_FLAG","MUTATION_DATE","BANK_ACCOUNT_ID","NO_SHIFT") VALUES (?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP,?,?,?,?,?)';
