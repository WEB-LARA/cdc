<?php
  class Mod_cdc_master_shift extends CI_Model{
    ...
    function insertMastershift($toko){
      ...
      //     $values  = implode(',', $d);
      //   //  $statement =  'INSERT into cdc_master_shift ("BRANCH_CODE","STORE_CODE","TGL_ACTIVE","TGL_INACTIVE","STATUS","TOTAL_SHIFT","TIPE_SETORAN","TIPE_SHIFT","CREATION_DATE","LAST_UPDATE_DATE","LAST_UPDATE_BY") VALUES ('.$values;

      //     print_r($d); // Change this to actually execute the statement
      // }
   ...
   function updateMasterShift($store_code,$start_date,$end_date,$tipe_shift,$jml_shift,$metode_setor){
        ...
        if($date2==''){
            $statement =  'UPDATE cdc_master_shift set "TGL_ACTIVE"=?,"TOTAL_SHIFT"=?,"TGL_INACTIVE"=NULL,"TIPE_SETORAN"=?,"TIPE_SHIFT"=?,"STATUS"=?,"LAST_UPDATE_DATE"=CURRENT_DATE WHERE "STORE_CODE"=?';
            $result=$this->db->query($statement,array($date,$jml_shift,$metode_setor,$tipe_shift,'A',$store_code));
             return $this->db->affected_rows();
        }else{
            ...
            $statement =  'UPDATE cdc_master_shift set "TGL_ACTIVE"=?,"TGL_INACTIVE"=?,"TOTAL_SHIFT"=?,"TIPE_SETORAN"=?,"TIPE_SHIFT"=?,"STATUS"=?,"LAST_UPDATE_DATE"=CURRENT_DATE WHERE "STORE_CODE"=?';
            $result=$this->db->query($statement,array($date,$date2,$jml_shift,$metode_setor,$tipe_shift,$activeFlag,$store_code));
             return $this->db->affected_rows();
        }
