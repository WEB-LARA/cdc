...
class Mod_deposit extends CI_Model {
	...
	public function get_tipe_shift($branch_code,$store_code,$rec_id){
	  ...
	  public function get_jumlah_shift($store_code,$sales_date)
    {

      $statement = 'SELECT "TOTAL_SHIFT" FROM cdc_master_shift WHERE "STORE_CODE" = ? AND "TIPE_SHIFT"=\'SS\'  and "TGL_ACTIVE"<=? AND ("TGL_INACTIVE" IS NULL OR "TGL_INACTIVE">=?)';
      $rs=$this->db->query($statement, array($store_code,$sales_date,$sales_date))->row();
      return $rs->TOTAL_SHIFT;
    }


   public function get_data_ftp($server)
   {
   		$statement='select * from cdc_master_ftp where "SERVER"=?';
   	  $rs=$this->db->query($statement, array($server))->row();
      return $rs;
	...
	public function get_flag_final($store_code,$sales_date){
		...
		$statement='SELECT "TIPE_SHIFT" from cdc_master_shift cms where "STORE_CODE"=? and "TGL_ACTIVE"<=? AND ("TGL_INACTIVE" IS NULL OR "TGL_INACTIVE">=?)';
		$rs=$this->db->query($statement,array($store_code,$sales_date,$sales_date))->row();
		
		if($rs){
			$tipe_shift=$rs->TIPE_SHIFT;
			if($tipe_shift=='SS'){
			$statement='SELECT "TOTAL_SHIFT" from cdc_master_shift cms where "STORE_CODE"=?  and "TGL_ACTIVE"<=? AND ("TGL_INACTIVE" IS NULL OR "TGL_INACTIVE">=?)';
			$rs=$this->db->query($statement,array($store_code,$sales_date,$sales_date))->row();
			$total_shift=$rs->TOTAL_SHIFT;

			
			
			$statement_receipt='SELECT COUNT(*)  as "CURRENT" FROM cdc_trx_receipts ctr where ctr."STORE_ID"=(SELECT "STORE_ID" FROM cdc_master_toko where "STORE_CODE"=?) AND ctr."ACTUAL_SALES_FLAG"=\'Y\' AND ctr."SALES_DATE"=? ';
	...
	public function get_batch_sum($deposit_id)
	{
		...
		$statement = '
					   ...
					   WHERE     cmt."STORE_ID" = ctr."STORE_ID"
					         ...
					         AND cmt."STORE_TYPE" = \'F\'
					         AND ctb."CDC_BATCH_TYPE" NOT LIKE \'%STL%\' 
