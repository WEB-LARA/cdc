...
//    echo '<br> RETURN :'.$return_val;
    ...
    from 
      (
        select 
          SUM(
            case when asd."SHIFT" = \'1\' AND ( select
                              cdspd."SALES_AMOUNT"
                            from
                              cdc_data_sales_per_denom cdspd
                            where
                              cdspd."SALES_DATE" = ?
                              and cdspd."STORE_CODE" = ?
                              and cdspd."SHIFT" = \'1\') != 0 then asd."SALES_AMOUNT" else 
                                    ( select cdspd."SALES_AMOUNT" from cdc_data_sales_per_denom
     cdspd where cdspd."SALES_DATE" = ? and cdspd."STORE_CODE" = ? and (cdspd."SHIFT" = \'H\' )) end) "SHIFT_1",
                            SUM(case when asd."SHIFT" = \'2\' AND ( select
                              cdspd."SALES_AMOUNT"
                            from
                              cdc_data_sales_per_denom cdspd
                            where
                              cdspd."SALES_DATE" = ?
                              and cdspd."STORE_CODE" = ?
                              and cdspd."SHIFT" = \'2\') >= 0 then asd."SALES_AMOUNT" else null end) "SHIFT_2",
                            SUM(case when asd."SHIFT" = \'3\' AND ( select
                              cdspd."SALES_AMOUNT"
                            from
                              cdc_data_sales_per_denom cdspd
                            where
                              cdspd."SALES_DATE" = ?
                              and cdspd."STORE_CODE" = ?
                              and cdspd."SHIFT" = \'3\') >= 0 then asd."SALES_AMOUNT" else null end) "SHIFT_3"
                          from
                            (
                            select
                              cdspd."SHIFT",cdspd."SALES_AMOUNT"
                            from
                              cdc_data_sales_per_denom cdspd
                            where
                              cdspd."SALES_DATE" = ?
      ...
      $result = $this->db->query($statement,array($sales_date,$store_code,$sales_date,$store_code,$sales_date,$store_code,$sales_date,$store_code,$sales_date,$store_code))->result();
    ...
    //       $whereSelisih = ' WHERE ABS((select coalesce("TOTAL_SALES_AMOUNT",0) - case when "SHIFT_FLAG" = \'N\' then "HARIAN" else (coalesce("1", 0)+ coalesce("2", 0)+ coalesce("3", 0)) end)) = 0 ';
    //     }else if($selisih == 'Selisih'){
    //       $whereSelisih = ' WHERE ABS((select coalesce("TOTAL_SALES_AMOUNT",0) - case when "SHIFT_FLAG" = \'N\' then "HARIAN" else (coalesce("1", 0)+ coalesce("2", 0)+ coalesce("3", 0)) end)) > 0 ';
    //     }else if($selisih == 'Selisih > 1000'){
    //       $whereSelisih = ' WHERE ABS((select coalesce("TOTAL_SALES_AMOUNT",0) - case when "SHIFT_FLAG" = \'N\' then "HARIAN" else (coalesce("1", 0)+ coalesce("2", 0)+ coalesce("3", 0)) end)) > 1000 ';
    //     }
    ...
    //                      AND cmaa."STORE_CODE" = cmt."STORE_CODE" LIMIT 1)
    //                as "AM",
...
public function get_data_monitoring_sales_fisik($start_periode,$end_periode,$branch_code,$branch_id,$kode_toko,$tipe_setoran,$selisih){
      if($branch_id != '100'){
        ...
        $statement='SELECT *,ABS((select coalesce("TOTAL_SALES_AMOUNT",0) - case when "SHIFT_FLAG" = \'N\' then "HARIAN" else (coalesce("1", 0)+ coalesce("2", 0)+ coalesce("3", 0)) end)) "SELISIH" 
        from (SELECT asd."STORE_ID",asd."BRANCH_CODE",asd."AM",asd."AS",asd."STORE_CODE",asd."STORE_NAME",asd."SALES_DATE", asd."SHIFT_FLAG",asd."HARIAN",asd."CREATE_DATE4",asd."1",asd."2",asd."3",
        (CASE WHEN asd."SHIFT_FLAG" = \'N\' THEN \'HARIAN\' else \'SHIFT\' END) "TIPE_INPUTAN",
        (CASE WHEN asd."SHIFT_FLAG" = \'N\' THEN asd."HARIAN" else (coalesce(asd."1",0)+coalesce(asd."2",0)+coalesce(asd."3",0)) END) "TOTAL_SALES",
        (case
          when (
        ...
        $statement='SELECT *,ABS((select coalesce("TOTAL_SALES_AMOUNT",0) - case when "SHIFT_FLAG" = \'N\' then "HARIAN" else (coalesce("1", 0)+ coalesce("2", 0)+ coalesce("3", 0)) end)) "SELISIH" from (SELECT asd."STORE_ID",asd."BRANCH_CODE",asd."AM",asd."AS",asd."STORE_CODE",asd."STORE_NAME",asd."SALES_DATE",asd."SHIFT_FLAG",asd."HARIAN",asd."CREATE_DATE4",asd."1",asd."2",asd."3",
        (CASE WHEN asd."SHIFT_FLAG" = \'N\' THEN \'HARIAN\' else \'SHIFT\' END) "TIPE_INPUTAN",
        (CASE WHEN asd."SHIFT_FLAG" = \'N\' THEN asd."HARIAN" else (coalesce(asd."1",0)+coalesce(asd."2",0)+coalesce(asd."3",0)) END) "TOTAL_SALES",
        (case
          when (
    ...
    //  public function get_count_data_monitoring_sales_fisik($start_periode,$end_periode,$branch_code,$branch_id,$kode_toko,$tipe_setoran,$selisih){
    //   if($branch_id != '100'){
    //     $whereKodetoko = '';
    //     if($kode_toko != '0000'){
    //       $whereKodetoko = ' AND cmt."STORE_CODE" = \''.$kode_toko.'\''; 
    ...
    //              ctrs."SHIFT_FLAG"
    //     ORDER BY cmt."STORE_ID", ctrs."SALES_DATE") asd)bcd'.$whereSelisih.$whereMetodeSetoranDana;
    //     $result = $this->db->query($statement,array($start_periode,$end_periode,$branch_code,$start_periode,$start_periode))->num_rows();
    //   }else{
    //     $whereKodetoko = '';
    //     if($kode_toko != '0000'){
    //       $whereKodetoko = ' AND cmt."STORE_CODE" = \''.$kode_toko.'\''; 
    ...
    //              ctrs."SHIFT_FLAG"
    //     ORDER BY cmt."STORE_ID", ctrs."SALES_DATE") asd)bcd'.$whereSelisih.$whereMetodeSetoranDana;
    //     $result = $this->db->query($statement,array($start_periode,$end_periode,$start_periode,$start_periode))->num_rows();
    //   }
    //   return $result;
    // }
...
AND \''.$from.'\' <=  coalesce(cs."TGL_INACTIVE_CABANG",\''.$from.'\') and "BRANCH_ID" = \''.$branchId.'\'
  ...
  public function get_data_tren_collection($date_tren)
  {
    $time = strtotime($date_tren);
    $final = date("Y-m-d", strtotime("+1 month", $time));
    $statement = 'SELECT CMA."AM_SHORT" NAMA_AM,
                         CMA."AS_SHORT" NAMA_AS,
                         CMT."STORE_CODE" KODE_TOKO,
                         CMT."STORE_NAME" NAMA_TOKO,
                         (SELECT MAX("TGL_KIRIM") FROM cdc_go_table WHERE "STORE_CODE" = CMT."STORE_CODE"  and "TGL_KIRIM">? and "TGL_KIRIM"<?) MAKS
                         FROM cdc_master_am_as CMA, cdc_master_toko CMT  WHERE CMA."STORE_CODE" = CMT."STORE_CODE" AND BTRIM(CMA."BRANCH_CODE") = BTRIM(?)  order by CMA."STORE_CODE"';
    return $this->db->query($statement,array($date_tren,$final,$this->session->userdata('branch_code')))->result();
  }

  public function cek_sales_pertanggal($kode_toko,$date,$month_year)
  {
    $statement = 'select count(*) CNT from cdc_trx_receipts where "SALES_DATE" = to_date(\''.$date.' \'||\''.$month_year.'\', \'DD MM YYYY\') AND "STORE_ID" = (SELECT "STORE_ID" FROM cdc_master_toko WHERE "STORE_CODE" = ? AND "BRANCH_ID" = (SELECT "BRANCH_ID" FROM cdc_master_branch where "BRANCH_CODE" =?)limit 1)  AND "ACTUAL_SALES_FLAG" = \'Y\'';
