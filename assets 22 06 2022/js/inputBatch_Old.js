...
function totalGTUInput() {
	...
	$.ajax({
		method: "POST",
		url: base_url+"InputBatch/getGrandTotal",
		success: function (total) {
...
function totalSetor(){
	...
	$.ajax({
		method: "POST",
		url: base_url+"InputBatch/getTotalSetor",
		success: function (total) {
	...
	$.ajax({
		method: "POST",
		url: base_url+"InputBatch/getTotalSetorF",
		success: function (total) {
...
$(document).ready(function(){
	...
	$('#data_trx_kurset').datagrid({
        url: base_url + 'InputBatch/get_data_kurset',
        striped: true,
        rownumbers:true,
		remoteSort:false,
		singleSelect:true,
		...
		$('#batch_reject').datagrid({
			...
			onDblClickRow: function () {
				...
				$('#tblTrxReceipts').datagrid({
					url:base_url+'InputBatch/getPraInputReject/'+rows[0].CDC_BATCH_ID,
					columns:[[
				...
				{field: 'BUTTON_EDIT', title: '' ,width:40 ,align:'center', formatter: function (value, row, index) {
					var col;
					col = ' <input type="button" id="btnEditEntry" value="Edit" onClick="editEntry('+row.CDC_REC_ID+',\''+row.STN_FLAG+'\')"> ';
					return col;
				}},

				{field: 'BUTTON_DELETE', title: '' ,width:50 ,align:'center', formatter: function (value, row, index) {
					var col;
					col = ' <input type="button" id="btnDelEntry" value="Delete" onClick="delEntry('+row.CDC_REC_ID+')"> ';
					return col;
				}}
				]],
	...
	*/		
	...
	$('#tblTrxReceipts').datagrid({
		url:base_url+'InputBatch/getPraInput',
		onCheckAll: function () {
			$("#genBatch").trigger('click');
		},
		columns:[[
				...
				if ($("#penam\\["+msg[i].TRX_PLUS_ID+"\\]").numberbox('getValue') != '') {
					if (parseInt($("#penam\\["+msg[i].TRX_PLUS_ID+"\\]").numberbox('getValue')) > 0) {
						if (validate_date($("#pendate\\["+msg[i].TRX_PLUS_ID+"\\]").datebox('getValue'))) {
							$.ajax({
								url: base_url+'input/Trx_Tambah/save_data_penambah/',
								type: 'POST',
								data: {
						...
						if ($("#penid\\["+msg[i].TRX_PLUS_ID+"\\]").val() != '') {
							$.ajax({
								url: base_url+'input/Trx_Tambah/delete_data_penambah/',
								type: 'POST',
								data: {
...
//POP UP WINDOW PENAMBAH
function penambah(){
	$("#input-penambah").window('open');
	var receipt_id = $('#CDC_REC_ID').val();
	var store = $("#storeCode").textbox('getValue');
	...
	$.ajax({
		...
		success: function (msg) {
			for (var i = 0; i < msg.length; i++) {
				...
				$.ajax({
					url: base_url+'input/Trx_Tambah/get_rec_penambah/',
					method: 'POST',
					dataType: 'json',
					data: {
						rec_id: receipt_id,
					...
					success: function (res) {
						if (res.length > 0) {
							$("#penid\\["+res[0].TRX_PLUS_ID+"\\]").val(res[0].TRX_DETAIL_ID);
			...
			for (var i = 0; i < msg.length; i++) {
				if ($("#kurset\\["+msg[i].TRX_MINUS_ID+"\\]").numberbox('getValue') != '') {
					if (parseInt($("#kurset\\["+msg[i].TRX_MINUS_ID+"\\]").numberbox('getValue')) > 0) {
						$.ajax({
							url: base_url+'input/Trx_Kurang/save_data_pengurang/',
							type: 'POST',
							data: {
						...
						if ($("#kurid\\["+msg[i].TRX_MINUS_ID+"\\]").val() != '') {
							$.ajax({
								url: base_url+'input/Trx_Kurang/delete_data_pengurang/',
								type: 'POST',
								data: {
	...
	$.ajax({
		...
		success: function (msg) {
			for (var i = 0; i < msg.length; i++) {
				...
				$.ajax({
					url: base_url+'input/Trx_Kurang/get_rec_pengurang/',
					type: 'POST',
					dataType: 'json',
					data: {
						rec_id: dataId,
					...
					success: function(res) {
						if (res.length > 0) {
							$("#kurid\\["+res[0].TRX_MINUS_ID+"\\]").val(res[0].TRX_DETAIL_MINUS_ID);
...
function btnGenerate(){
	...
	if(count >= 1){		
		...
		if (parseInt($("#role-id").val()) == 3 && parseInt(data_kur.total) > 0) {
			$.messager.confirm('Confirm','Input dengan GTU ?',function(x){
				if(x){
					$('#GTU_sent').dialog('open');
					$('#GTU_sent').dialog('center');
