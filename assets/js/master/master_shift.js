...
$(document).ready(function(){
	...
	columns:[[
		{field:'SHIFT_ID',			hidden:true},
		{field:'BRANCH',			title:'Branch',width:100,align:'center'},
		{field:'SHIFT_NUMBER',		title:'Shift Number',width:50,align:'center'},
		{field:'SHIFT_TIME_FROM',	title:'Shift Start',width:100,align:'center'},
		{field:'SHIFT_TIME_TO',		title:'Shift To',width:100,align:'center'},
		{field:'ACTIVE_FLAG',		title:'Active',width:25,align:'center'},
		{field:'INACTIVE_DATE',		title:'Inactive Date',width:100,align:'center'}
        ...
        $.ajax({
					  ...
					  success: function (message) {
						if(message!='0'){
							...
							$.messager.show({title: 'Success',msg: 'Data berhasil diupdate !'});
							$('#upload_new_store').dialog('close');

						}else{
							$.messager.show({title: 'Success',msg: 'Data gagal diupdate !'});
							$('#upload_new_store').dialog('close');
						}
...
function hapus(){
	...
	if (row){
		$.messager.confirm('Confirm','Are you sure you want to destroy this item?',function(r){
			if (r){
				$.post(base_url+"master/Shift/deleteData",{shiftId:row.BRANCH_ID},function(result){ 
					if (result.success){
