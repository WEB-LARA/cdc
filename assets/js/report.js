...
console.log(base_url_rep+'adt/'+branch+'/'+kode_toko+'/'+periode_mks+'/pdf');
  ...
  $("#sub_mks_pdf").linkbutton({
    onClick: function (event) {
      ...
      if (branch != '' && kode_toko != '' && status_setor != '' && periode_mks != '') {
        $.ajax({
          method: "POST",
          url: base_url+'report/get_count_mks/'+branch+'/'+kode_toko+'/'+status_setor+'/'+periode_mks+'/'+tipe_setoran+'/pdf',
          success: function (total) {
            if(total == '0'){
              $.messager.alert('Warning','Data yang dipilih tidak ada!');
            }else{
